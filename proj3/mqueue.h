...
private:
   ...
   ******************************************/
  Node* mergeHelper(Node* left, Node* right); //helper of mergeWithQueue
  void clear(Node* m_heap); //helper to deallocate all heap
  void printOrderHelper(Node *root) const;
  Node* copyHelper(Node* newNode, Node* source);
