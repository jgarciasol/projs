...
typedef unsigned int (*hash_fn)(string); //hash function 
...
class HashTable {
    ...
    int  m_transfers;  //amount transferred 
    int m_newIndex;  //index placeholder to know where we left off
    bool isRehashing;
    ...
    ******************************************/
    //void rehash(TABLENAME table);
    void rehash();
    bool insertHelper(File file);
};
#endif