...
int main(){
   Tester t;
   //tests insertion and removal on non colliding keys
   t.normalCon();
   //tests insertion on colliding keys
   t.collisionTest();
...
void Tester::normalCon(){
   ...
   for (int i = 0;i<14;i++)
         aTable.remove(File("mytest.cpp", tempDiskBlocks[i]));
   cout << "Message: dump after removing 14 buckets," << endl;
   aTable.dump();
}

void Tester::collisionTest(){
    Random diskBlockGen(DISKMIN,DISKMAX);
    HashTable aTable(MINPRIME,hashCode);
   int temp = 0;
   for (int i=0;i<55;i++){
    ...
    cout << "Message: dump after 55 insertions in a table with MINPRIME (101) buckets:" << endl;
    aTable.dump();
