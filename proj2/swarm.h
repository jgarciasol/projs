...
class Swarm{
    ...
    //helper function that will be used to insert a new bot. 
    //will be called by public insert(const Robot &robot)
    Robot* insert(int newBotID, Robot *currBot, ROBOTTYPE t, STATE s);

    //helper function to find bot
    bool findBot(Robot *aBot, int value) const;

    //helper function to listRobots()
    //takes in a Robot to traverse tree to print in ascending order 
    void listRobotsInOrder(Robot *aBot) const;

    //helper function to clear(); 
    void clear(Robot *aBot);

    //helper function to remove(int id)
    Robot* remove(Robot *aBot, int id);

    Robot* removeDead(Robot *aBot, STATE DEAD);
    //helper function to find min value in tree
    Robot* findMinBot(Robot *aBot);
    Robot* rightRotation(Robot *aBot);
    Robot* leftRotation(Robot *aBot);
    Robot* leftRightRotation(Robot *aBot);
    Robot *rightLeftRotation(Robot *aBot);
    Robot* returnBot(Robot* root, int id);
    // ***************************************************
};
#endif